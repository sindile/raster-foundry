...
object ProjectLayerDao extends Dao[ProjectLayer] {
  ...
  val selectF: Fragment =
    fr"SELECT id, created_at, modified_at, name, project, color_group, smart_layer, range_start, range_end, geometry" ++ tableF
  ...
  def insertProjectLayer(
    ...
    id, created_at, modified_at, name, project, color_group, smart_layer, range_start, range_end, geometry"
    )
    VALUES
    (${pl.id}, ${pl.createdAt}, ${pl.modifiedAt}, ${pl.name}, ${pl.project}, ${pl.colorGroup}, ${pl.smartLayer},
  ...
  def updateProjectLayerQ(projectLayer: ProjectLayer, id: UUID): Update0 = {
    ...
    val query = (fr"UPDATE" ++ tableF ++ fr"""SET
      ...
      colorGroup = ${projectLayer.colorGroup},
